...
class Pool extends EventEmitter {
  ...
  async handlePing(peer, packet) {
    const PongPacket = new packets.PongPacket(packet.nonce);
    await peer.sendMessage(PongPacket);
  }
...
class PoolOptions {
  ...
  fromOptions(options) {
    ...
    if (options.bip157 != null) {
      ...
      if (this.bip157)
        assert(
          this.filterIndexers && this.filterIndexers.size > 0,
          'Filter indexer is required for BIP 157');
    }
